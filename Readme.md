...
To test the CompactIE system on a set of sentences, first process those sentences as described [`here`](https://github.com/FarimaFatahi/CompactIE/tree/master/data/). Then, modify the ``config.yml`` file to change the path to the processed sentence file (`test_file`) and its conjunction file (`conjunctions_file`). Finally, run the above command to produce extractions. 
