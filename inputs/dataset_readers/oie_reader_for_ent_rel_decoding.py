...
class OIE4ReaderForEntRelDecoding():
    ...
    def get_wordpiece_tokens(self, line):
        ...
        # print("hh: ", results['wordpiece_tokens'])
    ...
    def get_entity_relation_label(self, line, sentence_length):
        # !! decided to just keep start and end of the spans although they maybe noncontinuous!!
        ...
        for entity in line['entityMentions']:
            ...
            if len(sub_spans) == 1:
                ...
                # idx2ent[entity['emId']] = (((st, ed + 1), ), entity['text'])
                # if len(sub_spans[0]) > 1:
                #     separate_positions.append(ed)
            else:
                # noncontinuous spans
        ...
        if 'entityLabelMatrix' not in line:
            logger.error("sentence id: {} doesn't contain 'entityLabelMatrix'.".format(line['sentId']))
            return False, results

        if 'relationLabelMatrix' not in line:
            logger.error("sentence id: {} doesn't contain 'relationLabelMatrix'.".format(line['sentId']))
            return False, results

        results['entity_label_matrix'] = line['entityLabelMatrix']
        results['relation_label_matrix'] = line['relationLabelMatrix']
