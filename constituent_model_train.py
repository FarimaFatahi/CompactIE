...
def train(cfg, dataset, model):
    ...
    for epoch, batch in dataset.get_batch('train', cfg.train_batch_size, None):
        ...
        batch['epoch'] = (epoch - 1)
        element_loss, symmetric_loss, implication_loss, triple_loss = step(cfg, model, batch, cfg.device)
...
def main():
    ...
    if cfg.test and os.path.exists('save_results/best_model'):
        state_dict = torch.load(open('save_results/best_model', 'rb'), map_location=lambda storage, loc: storage)
        model.load_state_dict(state_dict)
        logger.info("Loading best training model {} successfully for testing the constituent model.".format('save_results/best_model'))
